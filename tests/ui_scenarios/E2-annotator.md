...
## 2026-09-25 滑鼠停留自動切換工具

工具卡三顆工具鍵下方有「Switch tools by hover」開關（預設開、記在瀏覽器）。
規則：畫（Draw ✦）的時候把滑鼠停在任何一個控制點上 0.5 秒 → 切成 Edit 並選中那個形狀；
編輯（Edit）的時候把滑鼠停在空白處 0.7 秒（有形狀被選中時 1.8 秒，免得正要按 Delete 或點類別 chip
就被切走）→ 切回 Draw。滑鼠一動計時就重來，所以要「停著」才會切；按下滑鼠、離開畫布、
按 1/2/3 都取消計時。SAM 有點、框、預覽、parts 或已入列的物體時、或正在等類別選單時，不會切換
（切工具會丟掉這些狀態）。

1. 用「Done」佇列開一張有標記的照片，工具是 Draw ✦：把滑鼠移到某個形狀的控制點上——
   小圓點變白、游標變成手指；停 0.5 秒後 Edit 鍵亮起、該形狀被選中、控制點仍在游標下可直接拖
2. 停不到 0.5 秒就移開：仍是 Draw；在控制點附近晃動（每次移動都重新計時）：不切
3. Edit 且有選中形狀：把滑鼠停在空白處 1.2 秒仍是 Edit，2 秒後切回 Draw、選取取消；
   Edit 且沒有選中形狀：停 0.8 秒就切回 Draw
4. Edit 時滑鼠在空白處持續小幅移動 2 秒：不切；停下來 0.8 秒後才切
5. Draw 時先點了物件（有 SAM 預覽）再停在控制點上：不切；Esc 清掉後再停才會切
6. 取消「Switch tools by hover」：以上都不再發生，重新整理後仍是關閉；快捷鍵表（ⓘ）有一列說明

## 2026-09-25 右側面板照 mockup 重排（原本的扁平風格）

右欄工具卡的版面照使用者提供的 mockup 重排；同日試過的全站玻璃主題已依使用者要求撤回，
顏色、圓角、按鈕都維持頁面原本的扁平深色風格。

1. 開任一張照片：右欄第一張卡的標題「✦ Tools」下是三顆等寬的工具鍵（✦ Draw、鉛筆 Edit、
   襯線 T Prompt）；作用中那顆帶色框與淡色底（Draw／Prompt 綠、Edit 藍）；1／2／3 仍切換工具
2. Draw 工具：「Output」是分段開關，「Polygon」「Box」各帶一個圓形 radio 指示，選中那格藍字淡藍底、
   圓圈實心；右邊是 SAM 模型下拉（「SAM 2.1 Tiny」等短名稱，完整名稱在 tooltip）。
   切成 Box 後預覽變成外接矩形（行為同 SAM-T4 §9）
3. 「Points ≤」一列：共用的步進器，右側仍顯示「n points」
4. 三顆動作鍵一列：「✓ Accept」主按鈕（有 mask 才啟用，數量顯示成「Accept 3」／「Accept · 3 parts」，
   Enter 提示在 tooltip）、「＋ Part」、「🗑 Clear」（tooltip 註明 Esc）；其下整寬的「⬡ Boxes to polygons」；
   狀態列只在有事發生時顯示文字（準備影像、警告），準備好後是空的，沒有「ready」字樣
5. Prompt 工具：輸入框 → Output 分段開關（Box／Polygon）→ Confidence 滑桿＋步進器 → 整寬「✓ Apply」
6. 分隔線下方「🏷 Class」：輸入框右端有下拉箭頭，點進去（或按 ↓）列出專案全部類別可直接選；
   仍可打新名稱建立類別。下面的類別 chips 左邊是實心小圓點（無光暈），選中的 chip 藍框藍字
7. 「Tools」標題仍可點擊收合工具設定（行為同 2026-09-17 那節）；標題右端箭頭轉向、備註文字照舊
8. 側欄寬 340px；其餘頁面外觀不變
